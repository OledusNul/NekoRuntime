...
public abstract class AbstractScene extends AbstractVisualComponent {
    ...
    private void showOverlay(Pane newOverlay, EventHandler<ActionEvent> onFinished) {
        ...
        if (!Objects.equals(newOverlay.getId(), "loginPane")){
            disable();
        }
    ...
    public void hideOverlay(double delay, EventHandler<ActionEvent> onFinished) {
        ...
        new SlideOutDown(currentOverlayNode).play();
    }
...
//        if (scene.getRoot().getId() != "loginPane"){
//
//        }
        if (disablePane == null) {
    ...
    private void sceneBaseInit() {
        ...
        if (header == null) {
            ...
            LookupHelper.<ButtonBase>lookupIfPossible(header, "#exit").ifPresent((b) -> b.setOnAction((e) -> currentStage.close()));
            LookupHelper.<ButtonBase>lookupIfPossible(header, "#minimize").ifPresent((b) -> b.setOnAction((e) -> currentStage.hide()));
            ...
            LookupHelper.<ButtonBase>lookupIfPossible(layout, "#deauth").ifPresent(b -> b.setOnAction((e) ->
